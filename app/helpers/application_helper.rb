module ApplicationHelper
  ...
  def deeds_for(options={})
    ...
    if options[:collection]
      ...
      deeds = Deed.joins(:collection).where(collections: {restricted: false}).includes(:page, :user, collection: [:works]).where(condition).order_by_recent_activity.limit(limit)
    end
